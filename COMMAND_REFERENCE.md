...
| `5a` | 16 | free push | RSSI / squelch per side (not acked). byte 3 bit `0x20` = scan **PARKED** (live-pinned 2026-07-13): sets when the hop stops on a carrier (before the audio gate), holds through signal + the dropout-delay DWELL, clears at the exact hop resume. byte 4 bit `0x40` = **FOCUS side, ABSOLUTE** (`0x40` = B, `0x00` = A; live-pinned 2026-07-13 incl. a side-swap test): the selected side, EXCEPT while the unselected side is the sole active receiver (tail-held ~2-3 s past squelch close). **NOT the audio-path owner** — ear+clip-proven: audio routing is a holder-keeps LATCH (first side to open keeps the audio through overlaps; transfers only when the holder releases while the other side receives; reopening does not reclaim) and ignores this bit during overlaps. byte 12 `0x02` = scan running |
