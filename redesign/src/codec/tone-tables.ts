// CTCSS / DCS tables — pure data shared by the decoder, the 2f tone-write builder, and the UI tone
// picker. Kept dependency-free so the browser bundle can import it (decode.ts pulls in fs via the
// settings table, so the UI can't import the decoder directly).

/** Standard CTCSS tone frequencies (ANSI/EIA), 1-indexed by the radio's tone byte. */
...
/** The DCS codes the radio offers, as decimal labels (displayed octal: D023, D754, …). */
export const DCS_CODES: readonly number[] = [
